...
router.post("/register",async(req,res)=>{
    const {name , email , pno , course , city , mno , whichyear , interest , rolemodel , password  , cpassword ,collage } =req.body;
if(!name  || !email  || !pno  || !course  || !city  || !mno  || !whichyear  || !interest || !rolemodel || !password || !cpassword || !collage){
    return res.status(504).json({massage:"plz filled the field properly"});
}
try{
    ...
    if(userExist){
        ...
        const user = new User({name , email , pno , course , password , cpassword });
